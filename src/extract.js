...
(function (ns) {
  ...
  function branchLinks() {
    const scope =
      document.querySelector('[data-component="PageHeader.Description"]') || document;
  ...
  function refFromLink(anchor) {
    ...
    const match = (anchor.getAttribute('href') || '').match(/^\/[^/]+\/[^/]+\/tree\/(.+)$/);
  ...
  function getPageAuthor() {
    const meta = document.querySelector('.gh-header-meta .author, .gh-header-meta a.author');
  ...
  function getRawMarkdown(root) {
    ...
    if (node && typeof node.body === 'string' && node.body.trim()) return node.body;
  ...
  ns.loadRawMarkdown = function loadRawMarkdown(root, timeoutMs = 3000) {
    ...
    const deferred = root.querySelector('.timeline-comment-actions details-menu[src]');
  ...
  ns.isBodyComment = function isBodyComment(root) {
    ...
    const outer = root.parentElement && root.parentElement.closest('[id^="issue-"]');
  ...
  ns.extractComment = function extractComment(root, pageContext) {
    ...
    const pageUrl = pageContext.kind === 'issue' ? pageContext.issueUrl : pageContext.prUrl;
    return {
  ...
  const FAILURE_TEXT_RE = /(failing|failed|failure|errored|cancelled|canceled|timed out)/i;

  const ANNOTATION_SELECTOR = '.check-annotation, [data-testid="check-annotation"]';
  ...
  function statusText(row) {
    ...
    return clean(meta ? meta.textContent : row.textContent).replace(/^[-–—·:]\s*/, '');
  }
  ...
  ns.findFailingChecks = function findFailingChecks() {
    ...
    for (const link of document.querySelectorAll(CHECK_LINK_SELECTOR)) {
      ...
      found.push({
        ...
        checkUrl: link.href,
        failureOutput: [statusText(row), annotations].filter(Boolean).join('\n\n'),
      });
    }
